...
def preprocessingclaimst(servdat,claimdat, servre, preoprefta, admsdat, metri, b, tempsort_d, metricsneedingleftmerge, sizefla):#, reference_optional=None):
    """
    ...
    """
    debug1, debug2, debug3, debug4, debug5 = ('orig','orig','orig','orig','orig')
    #################################
...
#-------------------------------------------------------------------------------------    
    ...
    # careful about whether 'inner' is problematic; 
        servdat=servdat.merge(claimdat,on='MRN',how='left')
        claimdat=None
        print('check 1')
        servdat.EVT_DATE.fillna(np.datetime64('1900-01-01'), inplace=True)
...
#    servdat=servdat[~pd.isnull(servdat['ICD{}_subcode'.format(b)])]  
    servdat.dropna(subset=['ICD{}_subcode'.format(b)],inplace=True) ###?? this is probably a problem
    debug5=servdat
    print(sys.getsizeof(servdat))
    print('add test date_a')
    servdat['TEST_DATE_a']=servdat['TEST_DATE'].values.astype('datetime64[D]')
    print(sys.getsizeof(servdat))
    return(debug1, debug2, debug3, debug4, debug5, servdat) 
